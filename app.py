...
def profile():
    ...
    if request.method == 'POST':
        ...
        users_collection.update_one({'_id': ObjectId(session['user_id'])}, 
                                    {'$set': {'MIS_NO': mis_no, 'email': email, 'Mobile_No': mobile_no}})
        
        flash('Profile updated successfully.', 'success')  # Use flash messages for displaying alerts
        return redirect(url_for('profile'))  # Redirect to profile page after updating
        
    user_data = users_collection.find_one({'_id': ObjectId(session['user_id'])})
    return render_template('myProfile.html', user_data=user_data)  # Pass user_data to the template
